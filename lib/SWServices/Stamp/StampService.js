...
class StampService extends Services {
	...
	StampV2(xml, callback, isb64 = false) {
		// //manera 1
		// if(this.renewToken()) {
		// 	this._authenticateAndSendRequest(xml, 'v2', callback, isb64);
		// } else {
		// 	if(isb64)
		// 		StampRequest.sendReqB64(this.get_url(), this.get_token(), xml, 'v2', callback);
		// 	StampRequest.sendReq(this.get_url(), this.get_token(), xml, 'v2', callback);
		// }
		...
		// 	this._authenticateAndSendRequest(xml, 'v2', callback, isb64);
		// } else {
		// 	this._sendRequest(xml, 'v2', callback, isb64);
		// }
	...
	StampV3(xml, callback, isb64 = false) {
		...
		// if(isb64) 
		// 	StampRequest.sendReqB64(this.get_url(), this.get_token(), xml, 'v3', callback);
		// StampRequest.sendReq(this.get_url(), this.get_token(), xml, 'v3', callback);
	}

	StampV4(xml, callback, isb64 = false) {
		this._checkAndSend(xml, 'v4', callback, isb64);
		// if(isb64) 
		// 	StampRequest.sendReqB64(this.get_url(), this.get_token(), xml, 'v4', callback);
		// StampRequest.sendReq(this.get_url(), this.get_token(), xml, 'v4', callback);
