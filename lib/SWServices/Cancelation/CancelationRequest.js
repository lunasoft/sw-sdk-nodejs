...
class CancelationRequest {
	 ...
	 * Cancelación por CSD.
	 */
	static sendReqRetCSD(url, token, body, callback) {
		this._sendRequest(url, token, '/retencion/cancel/csd', body, callback);
	}

	/**
	 * Cancelación por PFX.
	 */
	static sendReqRetPFX(url, token, body, callback) {
		this._sendRequest(url, token, '/retencion/cancel/pfx', body, callback);
	}

	/**
	 * Cancelación por XML.
	 */
	static sendReqRetXML(url, token, xml, callback) {
		const delimiter = '-------------' + helper.uniqid();
		const fileFields = { xml: { type: "text/xml", content: xml } };
		let data = '';
		...
		this._sendRequest(url, token, '/retencion/cancel/xml', data, callback, { multipart: true, boundary: delimiter });
	}
