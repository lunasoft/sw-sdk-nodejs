...
class CancelationService extends Services {
	...
	constructor(params) {
		...
		// UUID
		if (params.rfc && params.uuid && params.motivo && 'folioSustitucion' in params) {
	...
	CancelationByUUID(callback) {
		this._checkAndSend(() => CancelationRequest.sendReqUUID(
			this.get_url(),
			this.get_token(),
			this._rfc,
			this._uuid,
			this._motivo,
			this._folioSustitucion,
			callback
		),
			callback
		);
	...
	CancelationByCSD(callback) {
		this._checkAndSend(() => CancelationRequest.sendReqCSD(
			this.get_url(),
			this.get_token(),
			this._body,
			callback
	...
	CancelationByPFX(callback) {
		this._checkAndSend(() => CancelationRequest.sendReqPFX(
			this.get_url(),
			this.get_token(),
			this._body,
			callback
	...
	CancelationByXML(callback) {
		this._checkAndSend(() => CancelationRequest.sendReqXML(
			this.get_url(),
			this.get_token(),
			this._xml,
			callback
	...
	_setUUID(params) {
		const required = ['url', 'token', 'rfc', 'uuid', 'motivo'];
		if (params.motivo === '01') { required.push('folioSustitucion'); }
		paramsMissing(required, params, 'UUID');
