...
class CancelationService {
	...
	CancelationByPFX(callback) {
		CancelationRequest.sendReqPFX(this.url, this.token, this._cfdiData, callback);
	}
	...
	_setUUID(params) {
		if(params.url && params.token && params.rfc && params.uuid && params.motivo) {
			...
			this._folioSustitucion = this.folioSustitucion;
		}else
	...
	_setPFX(params) {
		if(params.url && params.token && params.pfx && params.uuid && params.motivo && params.folioSustitucion){
			this._cfdiData = {
				url: url,
				token: token,
				pfx: pfx,
