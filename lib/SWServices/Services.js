...
class Services {
	...
	async _authenticate() {
		...
		const authSvc = Authentication.auth({
			user: this.get_user(),
		...
		const res = await new Promise((resolve, reject) => {
			authSvc.Token((err, data) => err ? reject(err) : resolve(data));
		});

		if (res.status !== 'success') {
			throw new Error(`Authentication error: ${res.message} Detail: ${res.messageDetail}`);
		}
