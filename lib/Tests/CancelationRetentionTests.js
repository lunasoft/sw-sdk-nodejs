const { assert } = require('chai');
const CancelationRetentionService = require('../SWServices/CancelationRetention/CancelationRetentionService');

const { SDKTEST_TOKEN, SDKTEST_USER, SDKTEST_PASSWORD } = process.env;

const SERVICES_URL = 'https://services.test.sw.com.mx';
const UUID = '2894d3ae-3387-4a60-bdd8-a2286b19021e';
const PASSWORD_CSD = '12345678a';
const PASSWORD_PFX = '12345678a';
const RFC = 'EKU9003173C9';
const B64CER = "";
const B64KEY = "";
const B64PFX = "";
const XMLRetention = "<?xml version='1.0' encoding='utf-8' ?><Cancelacion xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema' Fecha='2025-08-31T01:57:44' RfcEmisor='EKU9003173C9' xmlns='http://www.sat.gob.mx/esquemas/retencionpago/1'><Folios><Folio UUID='3044cc3f-572f-4535-85e2-374c205f5b11' Motivo='02' FolioSustitucion=''/></Folios><Signature xmlns='http://www.w3.org/2000/09/xmldsig#'><SignedInfo><CanonicalizationMethod Algorithm='http://www.w3.org/TR/2001/REC-xml-c14n-20010315'/><SignatureMethod Algorithm='http://www.w3.org/2000/09/xmldsig#rsa-sha1'/><Reference URI=''><Transforms><Transform Algorithm='http://www.w3.org/2000/09/xmldsig#enveloped-signature'/></Transforms><DigestMethod Algorithm='http://www.w3.org/2000/09/xmldsig#sha1'/><DigestValue>kd8XPP9pN8EnRyL6Wz0Tmty0gQA=</DigestValue></Reference></SignedInfo><SignatureValue></SignatureValue><KeyInfo><X509Data><X509Certificate></X509Certificate><X509IssuerSerial><X509IssuerName>CN=AC UAT, O=SERVICIO DE ADMINISTRACION TRIBUTARIA, OU=SAT-IES Authority, E=oscar.martinez@sat.gob.mx, STREET=3ra cerrada de caliz, PostalCode=06370, C=MX, ST=CIUDAD DE MEXICO, L=COYOACAN, OID.2.5.4.45=2.5.4.45, OID.1.2.840.113549.1.9.2=responsable: ACDMA-SAT</X509IssuerName><X509SerialNumber>3330303031303030303030353030303033343136</X509SerialNumber></X509IssuerSerial></X509Data></KeyInfo></Signature></Cancelacion>";

const EXPECTED_STATUS = 'success';

describe('CancelationRetentionServiceTest', function () {
	this.timeout(15000);

	const csdParams = {
		uuid: UUID,
		passwordCer: PASSWORD_CSD,
		rfc: RFC,
		b64Cer: B64CER,
		b64Key: B64KEY,
		motivo: '02',
		folioSustitucion: null
	};

	const pfxParams = {
		uuid: UUID,
		passwordPfx: PASSWORD_PFX,
		rfc: RFC,
		b64Pfx: B64PFX,
		motivo: '02',
		folioSustitucion: null
	};

	const xmlParams = {
		xml: XMLRetention
	};

	describe('CancelationByCSD', () => {
		it('Sucess con autenticación', done => {
			const params = {
				url: SERVICES_URL,
				user: SDKTEST_USER,
				password: SDKTEST_PASSWORD,
				...csdParams
			};

			CancelationRetentionService.Set(params).CancelationByCSD((err, res) => {
				assert.isNull(err, err && err.message);
				assert.equal(res.status, EXPECTED_STATUS);
				done();
			});
		});

		it('Sucess con token', done => {
			const params = {
				url: SERVICES_URL,
				token: SDKTEST_TOKEN,
				...csdParams
			};

			CancelationRetentionService.Set(params).CancelationByCSD((err, res) => {
				assert.isNull(err, err && err.message);
				assert.equal(res.status, EXPECTED_STATUS);
				done();
			});
		});
	});

	describe('CancelationByPFX', () => {
		it('Sucess con autenticación', done => {
			const params = {
				url: SERVICES_URL,
				user: SDKTEST_USER,
				password: SDKTEST_PASSWORD,
				...pfxParams
			};

			CancelationRetentionService.Set(params).CancelationByPFX((err, res) => {
				assert.isNull(err, err && err.message);
				assert.equal(res.status, EXPECTED_STATUS);
				done();
			});
		});

		it('Sucess con token', done => {
			const params = {
				url: SERVICES_URL,
				token: SDKTEST_TOKEN,
				...pfxParams
			};

			CancelationRetentionService.Set(params).CancelationByPFX((err, res) => {
				assert.isNull(err, err && err.message);
				assert.equal(res.status, EXPECTED_STATUS);
				done();
			});
		});
	});

	describe('CancelationByXML', () => {
		it('Sucess con autenticación', done => {
			const params = {
				url: SERVICES_URL,
				user: SDKTEST_USER,
				password: SDKTEST_PASSWORD,
				...xmlParams
			};

			CancelationRetentionService.Set(params).CancelationByXML((err, res) => {
				assert.isNull(err, err && err.message);
				assert.equal(res.status, EXPECTED_STATUS);
				done();
			});
		});

		it('Sucess con token', done => {
			const params = {
				url: SERVICES_URL,
				token: SDKTEST_TOKEN,
				...xmlParams
			};

			CancelationRetentionService.Set(params).CancelationByXML((err, res) => {
				assert.isNull(err, err && err.message);
				assert.equal(res.status, EXPECTED_STATUS);
				done();
			});
		});
	});
});
