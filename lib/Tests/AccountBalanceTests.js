...
describe('AccountBalanceServiceTest', function () {
	...
	describe('AddStamps', function () {
		it('Success con token', done => {
			...
			AccountBalanceService.Set(params).AddStamps(VALID_ID, NUMBER_OF_STAMPS, COMMENT, (err, res) => {
				assert.isNull(err);
				assert.equal(res.status, 'success');
		...
		it('Error con usuario inválido', done => {
			...
			AccountBalanceService.Set(params).AddStamps(INVALID_ID_ADD, NUMBER_OF_STAMPS, COMMENT, (err, res) => {
				assert.isNotNull(err);
	...
	describe('RemoveStamps', function () {
		it('Success con token', done => {
			...
			AccountBalanceService.Set(params).RemoveStamps(VALID_ID, NUMBER_OF_STAMPS, COMMENT, (err, res) => {
				assert.isNull(err);
				assert.equal(res.status, 'success');
				assert.isNumber(res.data, 'Data debe ser un número');
				done();
		...
		it('Error con usuario inválido', done => {
			...
			AccountBalanceService.Set(params).RemoveStamps(INVALID_ID_REM, NUMBER_OF_STAMPS, COMMENT, (err, res) => {
				assert.isNotNull(err);
				assert.equal(err.status, 'error');
				done();
			}
